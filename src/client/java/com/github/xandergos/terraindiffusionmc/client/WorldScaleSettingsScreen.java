 ...
 *
 * <p>26.2 confirmed API (via decompiled MC 26.2 jar):
 * <ul>
 *   <li>{@code CreateWorldScreen.getUiState()} → {@code WorldCreationUiState} (replaces
 *       old {@code getWorldCreator()}).</li>
 *   <li>{@code WorldCreationUiState.updateDimensions(DimensionsUpdater)} where
 *       {@code DimensionsUpdater} extends {@code BiFunction<RegistryAccess.Frozen,
 *       WorldDimensions, WorldDimensions>} — replaces old {@code applyModifier}.</li>
 *   <li>{@code WorldDimensions} is in {@code net.minecraft.world.level.levelgen}
 *       (not {@code .dimension}). Constructor takes
 *       {@code Map<ResourceKey<LevelStem>, LevelStem>}.</li>
 *   <li>{@code LevelStem} (replaces Yarn {@code DimensionOptions}): {@code type()}
 *       → {@code Holder<DimensionType>}, {@code generator()} → {@code ChunkGenerator}.</li>
 *   <li>{@code Screen.close()} → {@code Screen.onClose()} in Mojang mappings.</li>
 *   <li>26.2 GUI reorg: {@code Minecraft.setScreen} → {@code Minecraft.gui.setScreen}.</li>
 * </ul>
 */
public final class WorldScaleSettingsScreen extends Screen {
     ...
     *
     * <p>26.2: {@code getUiState().updateDimensions(BiFunction)} replaces the old
     * {@code getWorldCreator().applyModifier(BiFunction)}. The lambda receives a
     * {@code RegistryAccess.Frozen} and the current {@code WorldDimensions}, and
     * returns a modified {@code WorldDimensions}.
     */
    ...
    private WorldDimensions updateOverworldDimensionType(
            HolderGetter<DimensionType> dimensionTypeLookup,
        ...
        Holder<DimensionType> selectedDimensionTypeEntry = dimensionTypeLookup.get(dimensionTypeKey).orElse(null);
