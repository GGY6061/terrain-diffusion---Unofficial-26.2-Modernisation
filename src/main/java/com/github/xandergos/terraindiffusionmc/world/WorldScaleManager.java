...
public final class WorldScaleManager {
     ...
     *
     * <p>26.1 migration: {@code ServerWorld} → {@link ServerLevel},
     * {@code getPersistentStateManager().getOrCreate(TYPE)} →
     * {@code getDataStorage().computeIfAbsent(TYPE)} ({@code DimensionDataStorage}
     * was renamed to {@code SavedDataStorage} and exposed on both the server and
     * the level).
     */
